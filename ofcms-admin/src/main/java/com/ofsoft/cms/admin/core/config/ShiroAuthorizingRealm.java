...
public class ShiroAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		user = SysUser.dao
				.findFirst(
						"select u.*,r.role_id from syk_sys_user u left join syk_sys_user_role r on u.user_id = r.user_id where login_name = ? ",
						username);
		// 账号不存在
		if (user == null) {
	...
	private void setSession(Object key, Object value) {
		...
		if (null != currentUser) {
			...
			if (null != session) {
				...
				CacheKit.put(AdminConst.USER_ONLINE_KEY, session.getId(),
						session);
			}
		}
	}
